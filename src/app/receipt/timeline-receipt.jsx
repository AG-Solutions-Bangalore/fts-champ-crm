...
import { History, Mail, FileText, Download, Calendar, User, Printer, PlusCircle, Phone } from "lucide-react";
  ...
  const mapApiDataToUiFormat = (apiData) => {
    ...
    return apiData.map((item, index) => {
      ...
      if (item.followup_heading?.toLowerCase().includes('email')) {
        ...
        icon = Mail;
        color = "bg-blue-500";
      } else if (item.followup_heading?.toLowerCase().includes('receipt created')) {
        type = "creation";
        icon = PlusCircle;
        color = "bg-green-500";
