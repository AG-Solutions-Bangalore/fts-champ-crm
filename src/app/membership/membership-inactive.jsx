...
const MemberShipInactive = () => {
  ...
  const {
    ...
    queryKey: ["member-inactive", debouncedSearchTerm, pagination.pageIndex + 1],
    queryFn: async () => {
  ...
  useEffect(() => {
    ...
    if (currentPage > 1) {
      ...
      if (!queryClient.getQueryData(["member-inactive", debouncedSearchTerm, prevPage])) {
        queryClient.prefetchQuery({
  ...
  const handlePageChange = (newPageIndex) => {
    ...
    const cachedData = queryClient.getQueryData(["member-inactive", debouncedSearchTerm, targetPage]);
