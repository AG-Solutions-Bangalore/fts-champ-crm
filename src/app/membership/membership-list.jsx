...
const MembershipList = () => {
  ...
  const [whatsappSending, setWhatsappSending] = React.useState({});
  ...
  const handleExportCSV = () => {
    ...
    try {
      ...
      const csvData = memberData.members.map((member, index) => [
        ...
        member.indicomp_type === "Individual"
          ? member.indicomp_spouse_name
          : member.indicomp_com_contact_name || "",
        ...
        member.last_payment_vailidity
          ? `31-3-${member.last_payment_vailidity}`
          : "",
        member.chapter_name || "",
      ]);
  ...
  const columns = [
    ...
    },
    {
      ...
      header: "Mobile",
      cell: ({ row }) => (
        <div className="text-xs font-mono">{row.getValue("Mobile")}</div>
      ),
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div
            className={`text-xs ${isValidEmail ? "text-blue-600" : "text-gray-400"}`}
          >
    ...
    },
    {
      ...
      header: "Chapter",
      cell: ({ row }) => (
        <div className="text-xs">{row.getValue("Chapter")}</div>
      ),
    },
    ...(userType !== "4"
      ? [
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-gray-600">
                {memberData.members.length} member
                {memberData.members.length !== 1 ? "s" : ""}
              </p>
            </div>
          </div>
          <Button
