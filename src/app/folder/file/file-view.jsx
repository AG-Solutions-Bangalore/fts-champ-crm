...
const FileView = () => {
  const { fileUrl, fileName, folderunique } = useParams();
...
//   const [selectedColIndex, setSelectedColIndex] = useState(null);
  ...
  const { trigger: UpdateFileFolder, loading: isLoading } = useApiMutation();
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await UpdateFileFolder({
      ...
      if (code === 200) {
        toast.success(msg || "File updated successfully");
      }
