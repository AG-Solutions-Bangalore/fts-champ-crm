...
const DonorList = () => {
  ...
  // Get stored page from cookie or default to 0
  const storedPage = Cookies.get('donorlistrp');
  const initialPage = storedPage ? parseInt(storedPage) : 0;

  const [pagination, setPagination] = useState({
    pageIndex: initialPage, // Use the page from cookie
    pageSize: 10,
  });

  const [pageInput, setPageInput] = useState("");

  // Store current page in cookie whenever it changes
  useEffect(() => {
    Cookies.set('donorlistrp', pagination.pageIndex.toString(), { expires: 1 }); // Expires in 1 day
  ...
  // Navigation functions - set cookie before navigating
  const navigateToEdit = (id, donorType) => {
    // Page is already stored in cookie via useEffect above
  ...
  const handleEditClick = (id, donorType) => {
    navigateToEdit(id, donorType);
  };
  ...
  // Clear cookie when component unmounts (optional)
  useEffect(() => {
    return () => {
      // If you want to clear the cookie when leaving the page completely, uncomment below
      // Cookies.remove('donorlistrp');
    };
  }, []);
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const donorType = row.original.indicomp_type;
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => handleEditClick(id, donorType)}
                      >
                        <Edit />
                      </Button>
                    </TooltipTrigger>
    ...
    <div className="max-w-full p-2">
      <div className="flex items-center justify-between py-1">
        ...
        <div className="flex flex-col md:flex-row md:ml-auto gap-2 w-full md:w-auto">
          ...
          {userType !== '4' && (
            <>
              <Link to='/donor/donors-indiviusal-create'>
                <Button variant="default">
                  <SquarePlus className="h-3 w-3 mr-2" /> Indiviusal
                </Button>
              </Link>
              <Link to='/donor/donors-company-create'>
                <Button variant="default" >
