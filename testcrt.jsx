...
const SuperReceiptDonor = () => {
  ...
  const { trigger: updateDonor } = useApiMutation();
  const handleFetchReceipt = async () => {
    ...
    try {
      const res = await fetchReceipt({
        url: `/api/fetch-change-receipts-donor`,
        method: "post",
        data: { receipt_ref_no: receiptRefNo },
      });
      if (res.code === 200) {
        setReceiptData(res);
        toast.success(res?.msg || "Receipt fetched successfully");
      } else {
  ...
  } = useGetMutation("donorsfetch", `/api/fetch-change-donors`, {
  ...
  const handleUpdateDonor = async () => {
    ...
    try {
      const res = await updateDonor({
        url: `/api/update-change-receipts-donor/${receiptData.receipts[0].id}`,
        method: "put",
        data: {
    ...
    <div className="max-w-screen-lg mx-auto p-4 space-y-6">
      {!receiptData && (
        <motion.div
          ...
          className="flex flex-col items-center justify-center min-h-[70vh] px-4 bg-gray-50"
        >
          {/* Card Container */}
          <motion.div
            ...
            className="w-full max-w-xl bg-white rounded-3xl shadow-2xl border border-primary/20 p-8 md:p-10 flex flex-col items-center"
          >
            {/* Header */}
            <motion.div
            ...
            >
              <h1 className="text-xl md:text-2xl font-extrabold text-primary mb-2">
                Update Donor Info
              </h1>
              <p className="text-gray-600 text-sm md:text-base">
                Enter the receipt reference number to fetch donor details
              </p>
            </motion.div>
            ...
            >
              <motion.div
                ...
                className="absolute left-4 text-primary/70"
              >
                <File className="w-6 h-6" />
              </motion.div>

              <Input
                className="flex-1 pl-14 py-4 text-center text-lg md:text-xl border-none rounded-xl focus:ring-2 focus:ring-primary placeholder:text-gray-400 bg-primary/10"
              ...
              <Button
                className={`px-5 py-3 rounded-xl text-white font-semibold flex items-center justify-center gap-2 transition-all ${
                  loadingrecepit
            ...
            >
              Press Enter or click Search to fetch the receipt
            </motion.div>
            ...
            >
              <motion.div
                ...
                className="flex items-center justify-center gap-2 bg-primary/20 text-primary px-5 py-3 rounded-xl font-medium shadow-lg transition"
              >
              ...
              <motion.div
                whileHover={{ y: -5, scale: 1.05 }}
                className="flex items-center justify-center gap-2 bg-primary/30 text-primary-dark px-5 py-3 rounded-xl font-medium shadow-lg transition"
              >
